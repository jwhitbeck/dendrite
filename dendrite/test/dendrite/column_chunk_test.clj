...
(ns dendrite.column-chunk-test
  ...
  (:import [dendrite.java LeveledValue IOutputBuffer]
           ...
           [dendrite.column_chunk IColumnChunkReader IColumnChunkWriter]
...
(defn- colspec [value-type encoding compression]
...
(defn- colspec-non-repeated [value-type encoding compression]
  (metadata/map->ColumnSpec
   {:type value-type
    :encoding encoding
    :compression compression
...
(defn- colspec-required [value-type encoding compression]
  (metadata/map->ColumnSpec
   {:type value-type
    :encoding encoding
    :compression compression
...
(deftest data-column-chunk
  (let [cs (colspec :int :plain :deflate)
...
(deftest dictionary-column-chunk
  (let [cs (colspec :int :dictionary :deflate)
...
(deftest frequency-column-chunk
  (let [cs (colspec :int :frequency :deflate)
        input-blocks (->> #(helpers/rand-int-bits 10) repeatedly (rand-blocks cs) (take 1000))
        reader (write-column-chunk-and-get-reader cs input-blocks)
        output-blocks (flat-read reader)]
...
(deftest find-best-boolean-encodings
  (testing "random booleans"
    (let [cs (colspec-required :boolean :plain :none)
...
(deftest find-best-int-encodings
  ...
  (testing "random ints (non-repeated)"
    (let [cs (colspec-non-repeated :int :plain :none)
  ...
  (testing "small selection of chars"
    (let [cs (colspec-required :char :plain :none)
...
(deftest find-best-long-encodings
  (testing "random longs"
    (let [cs (colspec-required :long :plain :none)
  ...
  (testing "incrementing dates as a custom-type"
    (let [cs (colspec-required :inst :plain :none)
...
(deftest find-best-float-encodings
  (testing "random floats"
    (let [cs (colspec-required :float :plain :none)
...
(deftest find-best-double-encodings
  (testing "random doubles"
    (let [cs (colspec-required :double :plain :none)
...
(deftest find-best-byte-array-encodings
  ...
  (testing "small set of keywords"
    (let [cs (colspec-required :keyword :plain :none)
          input-blocks (->> #(rand-nth [:foo ::bar :baz]) repeatedly (take 1000))
          reader (write-column-chunk-and-get-reader cs input-blocks)]
      (is (= (flat-read reader) input-blocks))
      (is (= :dictionary (find-best-encoding* reader)))))
  (testing "small set of symbols"
    (let [cs (colspec-required :symbol :plain :none)
          input-blocks (->> #(rand-nth ['foo 'bar 'baz]) repeatedly (take 1000))
          reader (write-column-chunk-and-get-reader cs input-blocks)]
      (is (= (flat-read reader) input-blocks))
      (is (= :dictionary (find-best-encoding* reader))))))
...
(deftest find-best-fixed-length-byte-array-encodings
  (testing "random byte arrays"
    (let [cs (colspec-required :fixed-length-byte-array :plain :none)
          input-blocks (->> #(helpers/rand-byte-array 16) repeatedly (take 1000))
          reader (write-column-chunk-and-get-reader cs input-blocks)]
      (is (= (map seq (flat-read reader)) (map seq input-blocks)))
      (is (= :plain (find-best-encoding* reader)))))
  (testing "UUIDs"
    (let [cs (colspec-required :uuid :plain :none)
