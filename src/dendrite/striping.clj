  ...
  (let [definition-level (if (empty? record) definition-level (inc definition-level))]
    (reduce (fn [striped-record field]
              (let [v (get record (:name field))]
                (recursively-stripe-record striped-record v field (conj parents (:name field)) (empty? record) coercion-fns repetition-level definition-level)))
            striped-record
...
(defmethod recursively-stripe-record :seq
  ...
  (let [schema-without-repetition (assoc schema :repetition :optional)]
    (if (empty? record)
      (recursively-stripe-record striped-record nil schema-without-repetition parents true coercion-fns repetition-level definition-level)
      (reduce (fn [striped-record [sub-record repetition-level]]
                (recursively-stripe-record striped-record sub-record schema-without-repetition parents false coercion-fns repetition-level definition-level))
              striped-record
...
(defmethod recursively-stripe-record :map
  ...
  (let [schema-as-list (assoc schema :repetition :list)]
    (if (empty? record)
      (recursively-stripe-record striped-record nil schema-as-list parents true coercion-fns repetition-level definition-level)
      (recursively-stripe-record striped-record (map (fn [[k v]] {:key k :value v}) record) schema-as-list parents false
